...
Dependiendo de la política y los permisos que haya agregado a su cuenta de fábrica de carga de trabajo, puede implementar el sistema de archivos FSx para ONTAP completamente utilizando la fábrica de cargas de trabajo (mediante el modo de lectura/escritura). Si tienes menos permisos (modo de solo lectura) o no tienes permisos (modo básico), tendrás que usar la información de CloudFormation del CodeBox e implementar el sistema de archivos FSx para ONTAP tú mismo en AWS.
